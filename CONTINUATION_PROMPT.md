...
Branch: dev
Last Updated: April 13, 2026 (Tasks #61 + #62 + #63 — Architect-layer specialists renamed to `*_STRATEGIST` suffix so layer boundary is visible from the identifier; Scraper Specialist rebuilt as hybrid LLM + real-scraper Intelligence specialist; Competitor Researcher rebuilt as hybrid LLM + real Serper/DataForSEO/scraper Intelligence specialist with single multi-competitor cross-comparative analysis call. Intelligence dept 2/5 REAL, 23 total REAL specialists, 61% done)
...
**Current totals (as of April 13, 2026, after Tasks #61 + #62 + #63):**

| Category | Count | Notes |
|---|---|---|
| **REAL — confirmed AI agents** | **23** | Jasper, Copywriter, Video Specialist, Calendar Coordinator, Asset Generator, Growth Strategist, SEO Expert, LinkedIn Expert, TikTok Expert, Twitter/X Expert, Facebook Ads Expert, Growth Analyst, UX/UI Architect, Funnel Engineer, Workflow Optimizer, Copy Strategist, UX/UI Strategist, Funnel Strategist, Email Specialist, SMS Specialist, Scraper Specialist (Task #62), **Competitor Researcher (Task #63)** |
| **TEMPLATE — needs rebuild** | **15** | Hand-coded switch/lookup engines with zero LLM calls. |
...
| **NOT_WIRED — Jasper tools intentionally disabled** | **6** | `produce_video`, `orchestrate_campaign`, `delegate_to_sales`, `delegate_to_trust`, `delegate_to_intelligence`, `delegate_to_commerce` |
| **LIVE delegations** | **5** | `delegate_to_content` (Task #27), `delegate_to_marketing` (Task #34), `delegate_to_builder` (Task #38), `delegate_to_architect` (Task #42), **`delegate_to_outreach` (Task #45)** |
...
**Progress: 23 REAL / 38 total candidates (REAL + TEMPLATE) = 61% done.** Content department COMPLETE (4/4 REAL, `delegate_to_content` live). Marketing department COMPLETE (6/6 REAL, `delegate_to_marketing` live). Builder department COMPLETE (3/3 REAL, `delegate_to_builder` live). Architect department COMPLETE (3/3 REAL, `delegate_to_architect` live, all 3 renamed to `*_STRATEGIST` in Task #61). **Outreach department COMPLETE (2/2 REAL, `delegate_to_outreach` LIVE via Option B compose-only — Task #45, front-door pirate test PASSED both channels via Task #45.1 follow-up)** — Email Specialist ✓ (Task #43), SMS Specialist ✓ (Task #44). Voice AI Specialist remains classified INFRA (telephony wrapper, not an LLM rebuild candidate). **Intelligence department 2/5 REAL** — **Scraper Specialist ✓ (Task #62 — hybrid LLM + real scrapers, GM-backed analysis layer over existing `scrapeWebsite`/`scrapeAboutPage`/`scrapeCareersPage` infrastructure), Competitor Researcher ✓ (Task #63 — hybrid LLM + real Serper/DataForSEO/scraper, single multi-competitor call for cross-comparative market synthesis).** **5 of 11 Jasper delegations are now LIVE with end-to-end front-door verification.** **CROSS-CUTTING AUDIT COMPLETE (Task #45.2): all 18 LLM specialists are now pirate-test verified end-to-end against the provider honesty contract** (provider truncation lie fixed, every specialist's maxTokens aligned with its schema worst-case budget, every specialist's callOpenRouter has an explicit truncation backstop, 4 new pirate verifier scripts created for Marketing dept specialists that didn't have one, 3 schema cap widenings where the pirate tests surfaced Zod-too-tight bugs the truncation lie had been hiding). Next: 3 remaining Intelligence specialists (Technographic Scout Task #64, Sentiment Analyst Task #65, Trend Scout Task #66). When Intelligence is 5/5, rewire `delegate_to_intelligence` from NOT_WIRED to live.
...
**Intelligence department (5 specialists — 2/5 REAL, 3/5 TEMPLATE):**
