...
Branch: dev
Last Updated: April 14, 2026 (Task #59 — Alex / AI Chat Sales Agent rebuilt as pure LLM specialist with required GM, single action `respond_to_visitor`, stateless with optional caller-provided history + prior qualification, legacy taskType compat mapping for Jasper. Training Lab seed route inlined its own chat-widget directive — the two prompts (chat widget vs Jasper delegation JSON mode) are now properly separated. 27 total REAL specialists, 71% done.)
**Status: AGENT SPECIALIST REBUILD — 27/38 REAL (71%). Intelligence department COMPLETE (5/5, `delegate_to_intelligence` LIVE). Alex rebuilt (Task #59). 6 of 11 Jasper delegations live. Three departments remain: Sales (Tasks #46-#50, 5 specialists), Trust (Tasks #51-#54, 4 specialists), Commerce (Tasks #55-#58, 4 specialists).**
