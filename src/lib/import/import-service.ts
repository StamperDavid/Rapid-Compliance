...
import type { ColumnMapping, DetectedSchema, DetectedField, FieldTypeDetectionResult, ImportError } from '@/types/import';

export class ImportService {
  /**
   * Parse CSV file
   */
  static parseCSV(file: File): Promise<{ headers: string[]; rows: unknown[][] }> {
   ...
   */
  static detectFieldType(values: unknown[]): FieldTypeDetectionResult {
   ...
   */
  static detectSchema(headers: string[], rows: unknown[][], entityName: string): DetectedSchema {
  ...
  static validateData(
    ...
    rows.forEach((row, rowIndex) => {
      mappings.forEach(mapping => {
        ...
        // Validate by type
        if (value && value !== '') {
          if (mapping.fieldType === 'email' && !this.isValidEmail(String(value))) {
            errors.push({
              row: rowIndex + 2,
              column: mapping.csvColumn,
   ...
   */
  static transformValue(value: unknown, mapping: ColumnMapping): unknown {
    if (!value || value === '') {
