 ...
 * AI Video Factory - Sora, Runway integrations
 * (Kling Avatar via fal.ai is handled in the dedicated avatar API routes)
 *
...
export async function getVideoStatus(
  ...
  // Try each active provider if not specified
  if (!detectedProvider) {
    if (await getVideoProviderKey('sora')) {
