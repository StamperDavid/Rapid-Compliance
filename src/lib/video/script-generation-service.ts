...
### Scene Types:
1. **Presenter scenes (heygen)** — Avatar speaks on green screen. The backgroundPrompt drives a CINEMATIC VIDEO background generated by Runway or Kling. The two are composited together.
2. **B-roll scenes (runway or kling)** — Pure cinematic footage with NO avatar. scriptText MUST be empty "". Used for visual storytelling transitions, product close-ups, atmospheric shots.

### Engine selection:
- **heygen**: Assign to ALL scenes where the presenter speaks. The avatar is rendered on green screen and composited over a video background.
- **runway**: Assign to B-roll/cinematic inserts. Best for realistic motion, product shots, environment footage. scriptText must be empty.
- **kling**: Assign to B-roll that needs character consistency or full-body motion. Good for action sequences. scriptText must be empty.
...
## BACKGROUND PROMPTS (REQUIRED for every heygen scene)
