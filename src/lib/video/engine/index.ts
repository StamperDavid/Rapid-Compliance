...
export function generateVideo(
  ...
  // Generate storyboard
  const directorResponse = director.generateStoryboard(enhancedRequest);
...
export async function finalizeVideo(
  ...
  const job = stitcher.createJob(storyboard, generatedClips);
...
export function getEngineStatus(): {
  ...
  const picker = MultiModelPicker.getInstance();
  const _stitcher = StitcherService.getInstance();
