...
async function getRoundRobinAssignment(
  ...
  try {
    ...
    logger.warn('Round-robin assignment failed, using first user', error);
    return userIds[0];
  }
}
...
async function scheduleReminders(
  ...
  try {
    ...
    logger.warn('Failed to schedule reminders', error);
  }
}
...
Looking forward to speaking with you!
      ...
      if (attendee.phone) {
        ...
        await sendSMS({
          to: attendee.phone,
          body: `Reminder: ${meeting.title} at ${meeting.startTime.toLocaleTimeString()}. ${meeting.zoomJoinUrl || ''}`,
          organizationId,
        });
      }
    }
