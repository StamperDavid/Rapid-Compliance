...
export class NotificationService {
  ...
  private async getTemplate(templateId: string): Promise<NotificationTemplate | null> {
    try {
      const template = await AdminFirestoreService.get<NotificationTemplate>(
  ...
  private async getPreferences(userId: string): Promise<NotificationPreferences> {
    try {
      const prefs = await AdminFirestoreService.get<NotificationPreferences>(
  ...
  private async addToBatch(notification: Notification): Promise<void> {
    ...
    try {
      ...
      if (notificationIds.length >= maxPerBatch) {
        ...
        // Clear the batch
        await AdminFirestoreService.set(batchPath, batchKey, {
          notificationIds: [],
