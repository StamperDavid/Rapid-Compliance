...
function buildComposeWorkflowUserPrompt(req: ComposeWorkflowRequest): string {
  ...
  sections.push('- Use real agent IDs from the SalesVelocity.ai swarm when possible: SEO_EXPERT, LINKEDIN_EXPERT, TIKTOK_EXPERT, X_EXPERT, FACEBOOK_ADS_EXPERT, GROWTH_ANALYST, COPYWRITER, VIDEO_SPECIALIST, CALENDAR_COORDINATOR, ASSET_GENERATOR, UX_UI_ARCHITECT, FUNNEL_ENGINEER. If a required capability does not map to any existing agent, name the capability you would need (e.g. "TRANSLATION_SPECIALIST") and note in rationale that this agent does not yet exist.');
