...
const InstagramContentResultSchema = z.object({
  ...
  reelScript: z.object({
    ...
    closingCTA: z.string().min(5).max(300),
    audioSuggestion: z.string().min(5).max(200),
    estimatedDuration: z.string().min(2).max(20),
    trendReference: z.string().max(200).optional(),
  }).optional(),
  storySequence: z.array(z.object({
    ...
    interactiveElement: z.string().max(200).optional(),
  ...
  bioLinkStrategy: z.object({
    ...
    landingPageTip: z.string().min(10).max(300),
    linkInBioText: z.string().min(5).max(200),
  }),
});
