...
const AuditProfileResultSchema = z.object({
  ...
  napConsistency: z.object({
    ...
    issues: z.array(z.string().min(5).max(400)).max(15),
    recommendedActions: z.array(z.string().min(10).max(300)).max(15),
  }),
  categoryOptimization: z.object({
    primaryCategoryRecommendation: z.string().min(3).max(200),
    additionalCategoryRecommendations: z.array(z.string().min(3).max(200)).max(15),
    rationale: z.string().min(20).max(1500),
  }),
  mapPackReadiness: z.object({
    score: z.number().min(0).max(1),
    strengths: z.array(z.string().min(5).max(300)).min(1).max(15),
    weaknesses: z.array(z.string().min(5).max(300)).max(15),
    topImprovements: z.array(z.string().min(10).max(300)).min(1).max(15),
  }),
  competitiveAnalysis: z.object({
    marketPosition: z.enum(['leader', 'challenger', 'follower', 'niche']),
    differentiators: z.array(z.string().min(5).max(300)).max(15),
    gapsVsCompetitors: z.array(z.string().min(5).max(400)).max(15),
  }),
