/**
 * System Blueprint — Jasper's authoritative knowledge base.
 *
 * This is exported as a string constant so it gets compiled into the
 * serverless bundle. The previous approach used readFileSync + __dirname
 * which ALWAYS fails on Vercel because:
 *   1. __dirname resolves to the webpack output dir, not source
 *   2. .md files aren't included in the serverless function bundle
 *   3. readFileSync silently throws → falls through to stale hardcoded fallback
 *
 * By embedding the content here, query_docs always has the full blueprint.
 *
...
- **Providers:** Kling Avatar via fal.ai (avatar videos), Runway ML (cinematic B-roll)
