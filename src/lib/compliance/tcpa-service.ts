...
export async function addToSuppressionList(
  ...
  await AdminFirestoreService.set(SUPPRESSION_COLLECTION, suppressionId, record);

  // Also revoke consent
  const consentId = `${normalizedPhone}_${channel}`;
  try {
    await AdminFirestoreService.update(CONSENT_COLLECTION, consentId, {
      consentGiven: false,
...
export async function recordConsent(
  ...
  await AdminFirestoreService.set(CONSENT_COLLECTION, consentId, record);
