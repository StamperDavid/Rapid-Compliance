...
export default function SocialMediaTrainingPage() {
  ...
  const loadSettings = useCallback(async () => {
    try {
      ...
      const historyResult = await FirestoreService.getAllPaginated(
        `${COLLECTIONS.ORGANIZATIONS}/${orgId}/socialGenerationHistory`,
      ...
      const knowledgeResult = await FirestoreService.getAllPaginated(
        `${COLLECTIONS.ORGANIZATIONS}/${orgId}/socialKnowledge`,
      ...
      logger.error('Error loading social training settings:', error, { file: 'social/training/page.tsx' });
    } finally {
      setLoading(false);
    }
  }, [orgId]);
  ...
  const saveSettings = async () => {
    try {
      ...
      logger.error('Error saving social training settings:', error, { file: 'social/training/page.tsx' });
...
Generate ONLY the post content, keeping it under ${platformLimit} characters. Include relevant hashtags at the end.`;
      ...
      logger.error('Error generating sample post:', error, { file: 'social/training/page.tsx' });
  ...
  const copyToClipboard = async (text: string) => {
    try {
      ...
      logger.error('Failed to copy to clipboard:', error, { file: 'social/training/page.tsx' });
    }
  };
  ...
  const saveToHistory = async (post: GeneratedPost) => {
    try {
      ...
      logger.error('Error saving to history:', error, { file: 'social/training/page.tsx' });
    }
  };
