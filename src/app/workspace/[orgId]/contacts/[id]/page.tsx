...
export default function ContactDetailPage() {
  ...
  const loadContact = async () => {
    try {
      ...
      logger.error('Error loading contact:', error, { file: 'page.tsx' });
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="p-8">
      <div className="mb-6">
        ...
        <div className="flex items-start gap-6">
          <div className="w-20 h-20 rounded-full bg-blue-600 flex items-center justify-center text-3xl font-bold">{(contact.name || contact.firstName || 'U').charAt(0).toUpperCase()}</div>
          <div className="flex-1">
            <h1 className="text-3xl font-bold mb-2">{contact.name || `${contact.firstName} ${contact.lastName}`}</h1>
        ...
        <div className="col-span-2 space-y-6">
          <div className="bg-gray-900 rounded-lg p-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div><div className="text-sm text-gray-400 mb-1">Email</div><div>{contact.email || '-'}</div></div>
              <div><div className="text-sm text-gray-400 mb-1">Phone</div><div>{contact.phone || contact.phoneNumber || '-'}</div></div>
              <div><div className="text-sm text-gray-400 mb-1">Mobile</div><div>{contact.mobile || '-'}</div></div>
        ...
        <div className="space-y-6">
          <div className="bg-gray-900 rounded-lg p-6">
            ...
            <div className="space-y-2">
              ...
              <button 
                onClick={() => router.push(`/workspace/${orgId}/calls/make?phone=${encodeURIComponent(contact.phone || contact.phoneNumber)}&contactId=${contactId}`)}
