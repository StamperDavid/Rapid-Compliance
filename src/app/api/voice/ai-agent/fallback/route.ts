  ...
  try {
    ...
    if (callId && context) {
      try {
        ...
        logger.error('[AI-Fallback] Transfer notification failed:', transferError instanceof Error ? transferError : new Error(String(transferError)), {
          file: 'ai-agent/fallback/route.ts',
        });
      }
...
<Response>
  ...
  <Hangup/>
    ...
    logger.error('[AI-Fallback] Error in fallback handler:', error instanceof Error ? error : new Error(String(error)), {
      file: 'ai-agent/fallback/route.ts',
    });
