...
import { Shield, ArrowLeft, Lock, Loader2, Key, ShieldCheck, History } from 'lucide-react';
import { useAdminAuth } from '@/hooks/useAdminAuth';
import { logger } from '@/lib/logger/logger';
import type { Organization } from '@/types/organization';

export default function AdminOrgSecurityPage() {
    ...
    <div className="min-h-screen p-8" style={{ backgroundColor: '#000' }}>
      <div className="max-w-7xl mx-auto">
        ...
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} style={{ backgroundColor: '#1a1a2e', border: `1px solid ${primaryColor}40`, borderRadius: '0.75rem', padding: '1rem 1.5rem', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '1rem' }}>
          ...
          <div style={{ flex: 1 }}><div style={{ fontSize: '0.875rem', fontWeight: '600', color: '#fff' }}>Admin Support View</div><div style={{ fontSize: '0.75rem', color: '#666' }}>Security for: {orgLoading ? 'Loading...' : (organization?.name ?? orgId)}</div></div>
        ...
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-xl flex items-center justify-center shadow-lg" style={{ backgroundColor: '#ef4444' }}><Lock className="w-6 h-6 text-white" /></div>
            <div><h1 className="text-3xl font-bold text-white">Security Settings</h1><p className="text-gray-400">Authentication and access control</p></div>
          </div>
        </motion.div>
        {loading ? <div className="flex items-center justify-center py-20"><Loader2 className="w-8 h-8 animate-spin text-gray-400" /></div> : (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {[{ icon: Key, label: '2FA Status', value: 'Not Enabled', color: '#ef4444' }, { icon: ShieldCheck, label: 'SSO', value: 'Disabled', color: '#f59e0b' }, { icon: History, label: 'Audit Logs', value: 'Enabled', color: '#22c55e' }].map((item) => {
