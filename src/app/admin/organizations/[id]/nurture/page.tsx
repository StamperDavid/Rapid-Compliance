...
import { Shield, ArrowLeft, Heart, Loader2 } from 'lucide-react';
import { useAdminAuth } from '@/hooks/useAdminAuth';
import { logger } from '@/lib/logger/logger';
import type { Organization } from '@/types/organization';

/**
 * Admin Support View: Nurture Campaigns
 * View nurture campaigns for any tenant organization.
 */
export default function AdminOrgNurturePage() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        logger.error('Failed to load organization:', error instanceof Error ? error : new Error(String(error)), { file: 'admin/organizations/[id]/nurture/page.tsx' });
      } finally {
        setOrgLoading(false);
        setLoading(false);
      }
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} style={{ backgroundColor: '#1a1a2e', border: `1px solid ${primaryColor}40`, borderRadius: '0.75rem', padding: '1rem 1.5rem', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '1rem' }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: '0.75rem', color: '#666' }}>Nurture campaigns for: {orgLoading ? 'Loading...' : (organization?.name ?? orgId)}</div>
          </div>
        ...
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-bold text-white">Nurture Campaigns</h1>
              <p className="text-gray-400">Long-term relationship building campaigns</p>
            </div>
          </div>
        </motion.div>
        ...
        {loading ? (
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center py-16">
            ...
            <div className="text-xl font-semibold text-white mb-2">No nurture campaigns</div>
            <div className="text-gray-400">This organization has no nurture campaigns configured.</div>
          </motion.div>
        )}
      </div>
    </div>
