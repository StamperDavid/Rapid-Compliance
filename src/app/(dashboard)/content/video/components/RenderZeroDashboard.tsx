...
const StripItem = forwardRef<HTMLDivElement, StripItemProps>(function StripItem(
  { scene, index, isSelected, onSelect, onDelete, onDuplicate, onDragStart, onDrop },
  ref,
) {
      ...
      <div className="relative aspect-video bg-surface-elevated/50">
        {scene.screenshotUrl && !imgBroken ? (
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center text-muted-foreground">
            <ImageIcon className="w-6 h-6 mb-1" /><span className="text-[9px]">Scene {index + 1}</span>
          </div>
        )}
...
function PreviewPanel({ scene, onToggleReady }: {
  scene: PipelineScene;
  onToggleReady: () => void;
}) {
  ...
  return (
    <div className="space-y-3 lg:sticky lg:top-4 self-start">
      {/* This step is INFORMATION COLLECTION ONLY — no generation here. The live
          constructed prompt shows every captured detail; the actual render happens
          on the next step (Create Shot Doc → the shot doc). */}
...
/**
 * Assemble a complete brief from EVERY filled storyboard field so the Shot Doc
 * agent receives the full cinematic detail — nothing the operator entered is
 * dropped (script, action, setting, wardrobe, sound, and the full cinematic
 * preset prompt via composeBackgroundPrompt).
 */
function buildShotDocBrief(scenes: PipelineScene[]): string {
  if (scenes.length === 0) {
    return 'A short cinematic video.';
  }
  const blocks = scenes.map((s, i) => {
    const parts: string[] = [`SCENE ${i + 1}${s.title ? ` — ${s.title}` : ''}`];
    if (s.scriptText?.trim()) { parts.push(`Script / VO: ${s.scriptText.trim()}`); }
    if (s.visualDescription?.trim()) { parts.push(`Action: ${s.visualDescription.trim()}`); }
    if (s.location) { parts.push(`Location: ${s.location}`); }
    if (s.timeOfDay) { parts.push(`Time of day: ${s.timeOfDay}`); }
    if (s.weather) { parts.push(`Light / weather: ${s.weather}`); }
    if (s.wardrobe) { parts.push(`Wardrobe: ${s.wardrobe}`); }
    if (s.ambience) { parts.push(`Ambience: ${s.ambience}`); }
    if (s.musicCue) { parts.push(`Music: ${s.musicCue}`); }
    const look = composeBackgroundPrompt(s);
    if (look) { parts.push(`Camera & look: ${look}`); }
    if (s.duration) { parts.push(`Approx duration: ${s.duration}s`); }
    return parts.join('\n');
  });
  return [
    'Build a shot doc for the following video, scene by scene. Honor EVERY detail below exactly — every cinematic choice is intentional.',
    '',
    ...blocks,
  ].join('\n\n');
}

export function RenderZeroDashboard({
  onCreateShotDoc,
  onBack,
  isCreating = false,
  error: createError = null,
}: {
  onCreateShotDoc: (brief: string) => void;
  onBack?: () => void;
  isCreating?: boolean;
  error?: string | null;
}) {
    ...
    try {
      ...
      if (data.success && data.plan) {
        setDecompositionPlan({
          ...
          targetAudience: brief.targetAudience ?? '',
          keyMessages: [],
          scenes: data.plan.scenes.map((s) => ({ ...s, engine: 'fal' as const })),
          assetsNeeded: [],
  ...
  // Land straight on the deep fields: ensure one storyboard exists on entry, so
  // "Start blank / fill manually" opens directly on the controls (no extra
  // "Add storyboard" click). Runs once.
  ...
  useEffect(() => {
    ...
    if (useVideoPipelineStore.getState().scenes.length === 0) {
      handleAddScene(false);
    }
  }, [handleAddScene]);
  ...
  const handleToggleReady = useCallback(
    (sceneId: string) => {
      ...
      updateScene(sceneId, { status: scene.status !== 'approved' ? 'approved' : 'draft' });
    },
    [updateScene],
  );
  ...
  const handleCreateShotDoc = useCallback(async () => {
    if (!isReady) {
      return;
    }
    setIsSaving(true);
    ...
    // Hand the FULL filled-out detail to the Shot Doc agent (the joined design:
    // RenderZero form → Create Shot Doc → shot doc for review/generation).
    onCreateShotDoc(buildShotDocBrief(useVideoPipelineStore.getState().scenes));
  }, [isReady, updateScene, authFetch, projectId, projectName, brief, avatarId, avatarName, voiceId, voiceName, voiceProvider, setProjectId, onCreateShotDoc]);
  ...
  return (
    ...
    <div className="space-y-4 pb-28">
      ...
      <div className="flex justify-between items-center pt-2">
        ...
        <div className="flex items-center gap-2">
          {createError && (
            <span className="text-xs text-destructive max-w-xs truncate" title={createError}>
              {createError}
            </span>
          )}
          {onBack && (
          ...
          <Button
            ...
            disabled={!isReady || isSaving || isCreating}
            onClick={() => { void handleCreateShotDoc(); }}
          >
            {isSaving || isCreating ? <Loader2 className="w-4 h-4 animate-spin" /> : <CheckCircle2 className="w-4 h-4" />}
            {isSaving || isCreating ? 'Creating Shot Doc…' : 'Create Shot Doc'}
