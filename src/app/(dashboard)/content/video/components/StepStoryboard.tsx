...
export function StepStoryboard() {
  ...
  // Authoring view: the RenderZero storyboard strip, or the OpenArt-style
  // Shot Plan production sheet (both drive the same /content/video pipeline).
  const [viewMode, setViewMode] = useState<'strip' | 'sheet'>('strip');
  ...
  return (
    <div className="space-y-4 pb-28">
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          {/* View toggle: storyboard strip vs Shot Plan production sheet */}
          <div className="flex items-center gap-0.5 rounded-lg border border-border-strong bg-surface-elevated p-0.5" title="Switch between the storyboard strip and the Shot Plan production sheet">
            <button
              type="button"
              onClick={() => setViewMode('strip')}
              className={`flex items-center gap-1 px-2 py-1 rounded text-[11px] font-medium transition-colors ${viewMode === 'strip' ? 'bg-primary text-white' : 'text-muted-foreground hover:text-foreground'}`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-1 px-2 py-1 rounded text-[11px] font-medium transition-colors ${viewMode === 'sheet' ? 'bg-primary text-white' : 'text-muted-foreground hover:text-foreground'}`}
            >
              <ListVideo className="w-3 h-3" /> Shot Plan
            </button>
          </div>
      ...
      {/* Shot Plan production sheet — the OpenArt-style editable sheet view. */}
      {viewMode === 'sheet' && <ShotPlanSheet />}

      {/* Main: deep controls (left) + live preview (right) */}
      {viewMode === 'strip' && (scenes.length === 0 && !isDecomposing ? (
      ...
      {viewMode === 'strip' && scenes.length > 0 && (
      ...
      {/* Readiness */}
      {viewMode === 'strip' && missingRequirements.length > 0 && scenes.length > 0 && (
