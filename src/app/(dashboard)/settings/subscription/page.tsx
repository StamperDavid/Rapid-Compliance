...
export default function SubscriptionPage() {
  ...
  const loadSubscription = useCallback(async () => {
    try {
      ...
      logger.error('Failed to load subscription:', error instanceof Error ? error : new Error(String(error)), { file: 'page.tsx' });
    } finally {
      setLoading(false);
    }
  }, [orgId]);
  ...
  const toggleFeature = async (feature: string, enabled: boolean) => {
    try {
      ...
      logger.error('Failed to toggle feature:', error instanceof Error ? error : new Error(String(error)), { file: 'page.tsx' });
    }
  };
