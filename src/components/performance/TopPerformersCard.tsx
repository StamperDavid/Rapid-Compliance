...
export function TopPerformersCard({
  ...
  if (topPerformers.length === 0) {
    return (
      <div className="bg-surface-main rounded-lg shadow p-6">
        <h3 className="text-lg font-semibold text-[var(--color-text-primary)] mb-2">Top Performers</h3>
  ...
  return (
    <div className="bg-surface-main rounded-lg shadow">
      ...
      <div className="px-6 py-4 border-b border-border-light">
        <h3 className="text-lg font-semibold text-[var(--color-text-primary)]">Top Performers</h3>
        ...
        {topPerformers.map((performer, index) => (
          <div key={performer.repId}>
            ...
            <button
              ...
              className="w-full px-6 py-4 hover:bg-surface-elevated transition-colors text-left"
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  {/* Name and Score */}
                  <div>
                    <h4 className="font-semibold text-[var(--color-text-primary)]">{performer.repName}</h4>
                    <div className="flex items-center gap-3 mt-1">
                      <span className="text-sm text-[var(--color-text-secondary)]">
                        Score: <span className="font-semibold text-success">{performer.overallScore.toFixed(1)}</span>
                      </span>
                ...
                {/* Expand Icon */}
                <svg
                  className={`w-5 h-5 text-[var(--color-text-disabled)] transition-transform ${
            ...
            {expandedPerformer === performer.repId && (
              <div className="px-6 pb-4 bg-surface-elevated">
                {/* Top Strengths */}
                {performer.topStrengths.length > 0 && (
                  <div className="mb-4">
                    <h5 className="text-sm font-semibold text-[var(--color-text-primary)] mb-2">Top Strengths</h5>
                    <div className="space-y-2">
                      {performer.topStrengths.map((strength, idx) => (
                        <div key={idx} className="bg-surface-main p-3 rounded-lg border border-border-light">
                          <div className="flex items-center justify-between mb-1">
                            <span className="text-sm font-medium text-[var(--color-text-primary)]">{strength.area}</span>
                            <span className="text-sm font-bold text-success">{strength.score.toFixed(1)}</span>
                          </div>
                          <p className="text-xs text-[var(--color-text-secondary)]">{strength.description}</p>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {performer.standoutMetrics.length > 0 && (
                  <div className="mb-4">
                    <h5 className="text-sm font-semibold text-[var(--color-text-primary)] mb-2">Standout Metrics</h5>
                    <div className="space-y-2">
                      {performer.standoutMetrics.map((metric, idx) => (
                        <div key={idx} className="bg-surface-main p-3 rounded-lg border border-border-light">
                          <div className="flex items-center justify-between">
                            <span className="text-sm font-medium text-[var(--color-text-primary)]">{metric.metric}</span>
                            <span className="text-xs font-semibold text-primary">
                              {metric.percentageBetter.toFixed(0)}% better than team
                            </span>
                          </div>
                          <p className="text-xs text-[var(--color-text-secondary)] mt-1">{metric.description}</p>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {performer.successFactors.length > 0 && (
                  <div className="mb-4">
                    <h5 className="text-sm font-semibold text-[var(--color-text-primary)] mb-2">What Makes Them Great</h5>
