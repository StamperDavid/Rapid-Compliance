  ...
  return (
    <div className="bg-surface-main rounded-lg shadow">
      ...
      <div className="px-6 py-4 border-b border-border-light">
        <h3 className="text-lg font-semibold text-[var(--color-text-primary)]">Performance Trends</h3>
      ...
      <div className="p-6">
        ...
        {trendAnalysis.trendInsights.length > 0 && (
          <div className="mb-6 p-4 bg-surface-elevated rounded-lg border border-primary">
        ...
        {hasConsistent && (
          <div>
            <h4 className="text-sm font-semibold text-[var(--color-text-primary)] mb-3 flex items-center gap-2">
              <svg className="w-5 h-5 text-[var(--color-text-disabled)]" fill="currentColor" viewBox="0 0 20 20">
                <path fillRule="evenodd" d="M3 10a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1z" clipRule="evenodd" />
              </svg>
              Consistent Performers
            </h4>
        ...
        {!hasRisers && !hasFallers && !hasConsistent && (
          <div className="text-center py-8 text-[var(--color-text-disabled)]">
            <svg className="w-12 h-12 mx-auto mb-3 text-[var(--color-text-disabled)]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
function TrendBox({ label, trend }: TrendBoxProps) {
  ...
  return (
    <div className={`p-3 rounded-lg border ${getTrendColor(trend.direction)}`}>
      <div className="text-xs font-medium text-[var(--color-text-secondary)] mb-1">{label}</div>
...
function RepTrendRow({ rep, type }: RepTrendRowProps) {
  ...
  const getChangeColor = () => {
    if (rep.scoreChange > 0) {return 'text-success';}
    if (rep.scoreChange < 0) {return 'text-error';}
  ...
  return (
    <div className={`p-3 rounded-lg ${getBgColor()} flex items-center justify-between`}>
      <div className="flex items-center gap-3">
        {getIcon()}
        <div>
          <div className="font-medium text-[var(--color-text-primary)]">{rep.repName}</div>
          <div className="text-xs text-[var(--color-text-secondary)]">{rep.description}</div>
        </div>
      </div>
