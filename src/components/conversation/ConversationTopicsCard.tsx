...
export function ConversationTopicsCard({ analysis, className = '' }: ConversationTopicsCardProps) {
  ...
  const getImportanceBadge = (importance: string) => {
    const styles: Record<string, { bg: string; text: string }> = {
      ...
      low: { bg: 'bg-gray-100', text: 'text-[var(--color-text-secondary)]' },
    };
  ...
  const getSentimentColor = (sentiment: number) => {
    ...
    if (sentiment > -0.3) {return 'text-[var(--color-text-disabled)]';}
    return 'text-red-600';
  };
  
  return (
    <div className={`bg-surface-main rounded-lg shadow p-6 ${className}`}>
      <h2 className="text-xl font-semibold mb-4 text-[var(--color-text-primary)]">Topics & Discussion</h2>
      
      {/* Main Topics */}
      <div className="mb-6">
        <h3 className="text-sm font-semibold text-[var(--color-text-secondary)] mb-3">Topics Covered ({topics.mainTopics.length})</h3>
        
        {topics.mainTopics.length > 0 ? (
          <div className="space-y-3">
            {topics.mainTopics.slice(0, 5).map((topic, index) => (
              <div key={index} className="border-l-4 border-primary pl-3 py-2 bg-surface-elevated rounded-r">
                ...
                <div className="flex items-center gap-4 text-xs text-[var(--color-text-disabled)] mt-2">
                ...
                {topic.quotes.length > 0 && (
                  <div className="mt-2 text-sm text-[var(--color-text-disabled)] italic">
          ...
          <div className="text-sm text-[var(--color-text-disabled)] text-center py-4">
      ...
      {objections.length > 0 && (
        <div className="mb-6">
          <h3 className="text-sm font-semibold text-[var(--color-text-secondary)] mb-3">Objections ({objections.length})</h3>
          ...
          <div className="space-y-3">
            {objections.map((objection, index) => (
              ...
              >
                <div className="flex items-start justify-between mb-1">
                  <div>
                    <span className="font-medium text-[var(--color-text-primary)]">{objection.objection}</span>
                  ...
                  <span className="text-xs text-[var(--color-text-disabled)] capitalize">{objection.type}</span>
                </div>
                
                <div className="text-sm text-[var(--color-text-disabled)] italic mt-1">
                  {'"'}{objection.quote.substring(0, 120)}{objection.quote.length > 120 ? '...' : ''}{'"'}
                </div>
                
                {!objection.wasAddressed && objection.recommendedResponse && (
                  <div className="mt-2 p-2 bg-surface-main rounded border border-border-light">
                    <div className="text-xs font-medium text-[var(--color-text-secondary)] mb-1">Recommended Response:</div>
                    <div className="text-xs text-[var(--color-text-disabled)]">{objection.recommendedResponse}</div>
                  </div>
                )}
                
                {objection.wasAddressed && objection.responseQuality && (
                  <div className="mt-2 text-xs">
                    <span className="text-[var(--color-text-disabled)]">Response Quality: </span>
      ...
      {competitors.length > 0 && (
        <div className="mb-6">
          <h3 className="text-sm font-semibold text-[var(--color-text-secondary)] mb-3">Competitor Mentions ({competitors.length})</h3>
            ...
            {competitors.map((competitor, index) => (
              <div
                ...
                className={`border-l-4 pl-3 py-2 rounded-r ${
                  ...
                  'border-gray-500 bg-surface-elevated'
                }`}
              >
                <div className="flex items-start justify-between mb-1">
                  <span className="font-medium text-[var(--color-text-primary)]">{competitor.competitor}</span>
                  <div className="flex items-center gap-2">
                    <span className="text-xs text-[var(--color-text-disabled)]">{competitor.mentions} mention{competitor.mentions > 1 ? 's' : ''}</span>
                ...
                <div className="mt-2 p-2 bg-surface-main rounded border border-border-light">
                  <div className="text-xs font-medium text-[var(--color-text-secondary)] mb-1">Recommended Response:</div>
                  <div className="text-xs text-[var(--color-text-disabled)]">{competitor.recommendedResponse}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      {keyMoments.length > 0 && (
        <div>
          <h3 className="text-sm font-semibold text-[var(--color-text-secondary)] mb-3">Key Moments ({keyMoments.length})</h3>
          ...
          <div className="space-y-2">
            {keyMoments.slice(0, 5).map((moment, _index) => {
              ...
              const _impactColor =
                ...
                moment.impact === 'negative' ? 'text-red-600' : 'text-[var(--color-text-disabled)]';
              
              return (
                <div key={_index} className="flex items-start gap-3 p-3 bg-surface-elevated rounded">
                  <span className="text-lg">{impactIcon}</span>
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="font-medium text-[var(--color-text-primary)]">{moment.title}</span>
                      <span className="text-xs text-[var(--color-text-disabled)]">
                        {Math.floor(moment.timestamp / 60)}:{(moment.timestamp % 60).toString().padStart(2, '0')}
                      </span>
                    </div>
                    <p className="text-sm text-[var(--color-text-disabled)]">{moment.description}</p>
                    {moment.quote && (
                      <p className="text-sm text-[var(--color-text-disabled)] italic mt-1">
