 ...
 *
 * Single unified card for the central connected-Microsoft account. Replaces
 * the previous three fragmented entries (Outlook Mail, Outlook Calendar,
 * Microsoft Teams) — one OAuth grants access to all of them via the
 * MICROSOFT_FULL_SCOPE_BUNDLE, so showing them as separate "Connect"
 * buttons was misleading and visually inconsistent.
 *
 * Reads the connection state from /api/integrations/microsoft/status, which
 * goes through the Admin SDK and returns a narrow non-secret summary
 * (connected flag, accountEmail). Tokens never reach the client.
 *
 * Visual shape matches the GoogleServicesIntegration / Zoom / Slack
 * reference exactly: card chrome, icon + title + description, full-width
 * Connect button on the disconnected state, "Connected as <email>" +
 * service list + Disconnect button on the connected state. Disconnect is
 * two-step (per the destructive-actions standing rule).
 */
...
interface MicrosoftServicesProps {
  /**
   * Live connection state passed in by the parent integrations page.
   * Parent already calls /api/integrations/microsoft/status on load — we
   * accept it as a prop instead of fetching twice.
   */
  connected: boolean;
  accountEmail?: string;
  /** Trigger the parent to refresh its connection state after disconnect. */
  onRefresh: () => void;
}

const SERVICES_GRANTED = [
  { key: 'outlook-mail', icon: 'M', label: 'Outlook Mail — send & sync replies' },
  { key: 'outlook-calendar', icon: 'C', label: 'Outlook Calendar — events for every scheduled action' },
  { key: 'onedrive', icon: 'D', label: 'OneDrive — file storage' },
  { key: 'teams', icon: 'T', label: 'Microsoft Teams — channel posting' },
];

export default function MicrosoftServicesIntegration({
  connected,
  accountEmail,
  onRefresh,
}: MicrosoftServicesProps) {
  ...
  const handleConnect = async (): Promise<void> => {
    ...
    try {
      const res = await authFetch('/api/integrations/microsoft/auth');
      if (!res.ok) {
      ...
      logger.error(
        'Microsoft connect failed',
        error instanceof Error ? error : new Error(String(error)),
        { file: 'MicrosoftServicesIntegration.tsx' },
      );
  ...
  const handleDisconnect = async (): Promise<void> => {
    ...
    try {
      const res = await authFetch('/api/integrations/microsoft/disconnect', { method: 'POST' });
      if (!res.ok) {
        ...
        <div style={{ display: 'flex', alignItems: 'start', gap: '1rem', marginBottom: '1rem' }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <p style={{ fontSize: '0.875rem', color: 'var(--color-text-disabled)' }}>
              One sign-in connects Outlook Mail, Outlook Calendar, OneDrive, and Microsoft Teams
            </p>
          </div>
        </div>
        <button
