...
export function OrchestratorBase({ config }: { config: OrchestratorConfig }) {
  ...
  // The arrow-down toggle now opens Jasper's Insights inline (recommendations
  // generated by the INSIGHTS_ANALYST specialist from real platform data).
  // Was previously a 6-specialist quick-invoke palette that nobody used.
  const [showInsights, setShowInsights] = useState(false);
  const [insightsList, setInsightsList] = useState<Array<{
    id: string;
    title: string;
    summary: string;
  ...
  // Lazy-load insights when the operator opens the panel for the first time.
  // Once loaded for this session we cache; a manual refresh button could
  // call /generate?force=true later, but for now this is single-shot.
  useEffect(() => {
    if (!showInsights || insightsLoaded || insightsLoading) {
      return;
    }
    let cancelled = false;
    void (async () => {
      ...
      try {
        const res = await fetch('/api/jasper/insights', { credentials: 'include' });
        if (res.ok) {
          ...
          if (!cancelled && body.success === true && Array.isArray(body.insights)) {
            setInsightsList(body.insights);
          }
        }
      } catch {
        // Non-fatal — panel will show "no insights yet" empty state.
      } finally {
        if (!cancelled) {
  ...
  }, [showInsights, insightsLoaded, insightsLoading]);
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-6 right-6 z-50 w-[420px] h-[600px] flex flex-col rounded-2xl overflow-hidden backdrop-blur-xl bg-surface-paper border border-indigo-400/25 ring-1 ring-indigo-400/10 shadow-2xl shadow-indigo-500/10"
          >
            {/* Header */}
            ...
            {/* Jasper's Insights — proactive recommendations from the
                INSIGHTS_ANALYST specialist based on the operator's last
                7 days of platform activity. Click "Run this mission"
                on a card → drops the suggested prompt into the chat
                input so the operator can review and send. */}
            <AnimatePresence>
              {showInsights && (
                <motion.div
                  ...
                  exit={{ opacity: 0, y: 10 }}
                  className="border-t border-border-light bg-surface-paper backdrop-blur-md max-h-64 overflow-y-auto"
                >
                  <div className="px-3 pt-2 pb-1 flex items-center gap-2 sticky top-0 bg-surface-paper">
                    <Lightbulb className="w-3.5 h-3.5 text-indigo-300" aria-hidden />
                    <p className="text-xs font-semibold text-white">Jasper&apos;s Insights</p>
                  </div>
                  {insightsLoading && insightsList.length === 0 ? (
                    <p className="px-3 py-3 text-xs text-[var(--color-text-disabled)]">Loading…</p>
                  ) : insightsList.length === 0 ? (
                    <p className="px-3 py-3 text-xs text-[var(--color-text-disabled)]">
                      Nothing to flag right now — Jasper will check again tomorrow.
                    </p>
                  ) : (
                      ...
                      {insightsList.map((insight) => {
                        ...
                        return (
                          <button
                            key={insight.id}
                            onClick={() => {
                              setInput(insight.suggestedMissionPrompt);
                              setShowInsights(false);
                              inputRef.current?.focus();
                            }}
                            className="w-full text-left p-2 rounded-lg hover:bg-surface-elevated transition-colors group"
                          >
                            <div className="flex items-start gap-2">
                              <span className={`flex-shrink-0 inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-semibold uppercase ${urgencyClass}`}>
                                {insight.urgency}
                              </span>
                              <div className="flex-1 min-w-0">
                                <p className="text-white text-xs font-medium leading-tight">{insight.title}</p>
                                <p className="text-[var(--color-text-disabled)] text-[11px] mt-0.5 line-clamp-2">{insight.summary}</p>
                              </div>
                            </div>
                          </button>
                        );
            ...
            <div className="p-4 border-t border-border-light bg-surface-main">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  aria-label={showInsights ? "Close Jasper's Insights" : "Open Jasper's Insights"}
                  title="Jasper's Insights"
                  className={`p-2 rounded-lg transition-colors ${showInsights ? 'bg-primary text-white' : 'bg-white/5 text-[var(--color-text-secondary)] hover:bg-surface-elevated'}`}
                >
                  <ChevronDown className={`w-5 h-5 transition-transform ${showInsights ? 'rotate-180' : ''}`} />
                </button>
