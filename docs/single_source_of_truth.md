...
**Last Updated:** June 30, 2026 (**CRM Vertical #2 — all five remaining parity gaps built** in one parallel pass (5 subagents, disjoint file ownership; integrated in one serial list-page wiring pass): saved/filtered views with REAL server-side filtering, multiple pipelines + a `/pipelines` management screen, CSV import UI, editable record owner, and reporting surfaced in-CRM at `/crm-reports`. New routes: `/pipelines`, `/crm-reports`, `/api/crm/pipelines`, `/api/crm/views`, `/api/contacts/import`, `/api/deals/import`, `/api/crm/companies/import`. tsc + eslint clean; commit `a7a3275b` + pipeline-mgmt follow-up on `dev`, primary fast-forwarded. Certify each via an operator walkthrough — "done" = certified, not code-shipped. **Also Jun 30:** Video Tier-1 cinematic fixes ③ (cross-clip exposure + dialogue-volume normalization in the stitch — per-clip `normalize`/`loudnorm`, sample-rate relock so dialogue+silent clips still concat) + ④ (fake-4K upscale OFF by default; honest 1080p, Topaz kept behind `STITCH_4K_UPSCALE=1`) — Tier-1 now fully closed, `src/lib/video/shot-plan-generation-service.ts`. **SMS send failure diagnosed:** invalid Twilio credentials (HTTP 401 / error 20003), NOT a trial/verified-number issue — operator must refresh the Twilio Auth Token; read-only probe added at `scripts/diagnose-twilio-sms.ts`.)

**Prior Last Updated:** June 29, 2026 (**Navigation overhaul — Settings dissolved into the main menu; duplicates, orphans, and dead redirect-stubs purged. + Video Tier-1 cinematic fixes (frozen-tail + sound). + the Sales/Outreach "specialist hands" executors.** Commits on dev `112c3484`→`56b20bd8`.)

**Navigation consolidation (the bulk of this work).** The standalone Settings page was retired and every real config page was surfaced directly in the main left nav as purpose-grouped sections — five new entries in `NAV_SECTIONS` (`src/components/admin/AdminSidebar.tsx`): **Commerce** (Storefront, Promotions & Coupons, A/B Testing), **Brand & Templates** (Brand Identity, Music Library, Email Templates, SMS Messages, Scheduling Messages), **Automation** (Lead Routing, Inbound Automation), **Integrations** (Connected Apps, Accounting, API Keys, Webhooks, Custom Tools), **Organization** (Org Profile, Team Members, Security, Subscription & Billing, Features & Modules, Compliance Reports, Audit Log). `NavigationCategory` (`src/types/unified-rbac.ts`) extended with the 5 ids; each item keeps its original `requiredPermission` gating, so per-role visibility is unchanged. The config PAGES are untouched (same `/settings/*` URLs, so OAuth callbacks + login still work) — only how you reach them moved. Footer "Settings" → "My Account" (`/settings/account`); the `/settings` index is now a `redirect('/dashboard')` stub. **18 dead redirect-stub pages deleted** (`/scraper`, `/leads/{discovery,icp}`, `/content/{characters,locations,library,video/library}`, `/outbound`, `/sequences`, `/integrations`, `/ai-agents`, `/settings/{promotions,brand-dna,brand-kit,theme,workflows}`, `/analytics/pipeline`, `/website/seo/ai-search`), every inbound nav link repointed to the real target first (the two folders holding shared components — `content/characters/CharacterForm`, `content/video/library/AssetActionsMenu` — kept, only their `page.tsx` removed). **7 orphaned pages surfaced** into the menu: CRM ▸ Discovery (`/intelligence/discovery`), CRM ▸ Meetings (`/meetings`), CRM ▸ Document Templates (`/proposals`), Marketing ▸ Marketing Budget (`/marketing/budget`), AI Workforce ▸ Agent Setup (`/settings/ai-agents`). **4 duplicate sets resolved:** Activities (kept the fuller `/activities`; `/dashboard/activities` → redirect), Orders (kept the full `/orders`; `/deals/orders` → redirect), Tasks (kept the CRM `/tasks` queue view and ADDED its missing delete via `DELETE /api/team/tasks/[taskId]`; `/team/tasks` → redirect) were true duplicates and merged; Proposals and Campaigns were NOT duplicates (different tools sharing a name) so all were kept and renamed by purpose ("Document Templates" vs "Quotes"; "Social Media Campaigns" / "Email Campaigns" / "Campaigns"). **16 vestigial "Back to Settings" buttons/breadcrumbs scrubbed** from the settings sub-pages (+ now-unused Link imports). Orphaned `/content/video/studio` deleted (its tab was removed Jun 5; nothing linked it). Two read-only verification sweeps confirmed the end state: every menu item resolves to a real page, zero dangling links to deleted routes, zero remaining orphans or duplicates. tsc + lint clean on every commit.

**Video Tier-1 cinematic fixes** (`src/lib/video/shot-plan-generation-service.ts` + `ffmpeg-utils.ts`): (1) **frozen-tail stutter removed** — every shot used to get a 2s held-frame + 2s silence appended (`appendFrozenTail`) as an "overlap the editor trims," but nothing trimmed it, so a multi-clip film stuttered at every join; removed the append (cross-clip continuity still holds via the extracted last-frame chain `generated.lastFrameUrl`). (2) **Sound turned on** — `stitchShotPlan` now lays a mood-matched instrumental bed under the dialogue: one MusicGen track is generated per film from the plan's look bible (genre/mood/art-style), ducked beneath dialogue via the existing `mixAudioWithDucking` (sidechaincompress + LUFS) with a new `loopMusic` option (`-stream_loop -1`) so a 30s bed covers a multi-minute video; best-effort (no Replicate credit → ships un-scored, no regression). Remaining Tier-1 (see `CONTINUATION_PROMPT.md`): cross-clip color/volume match, and stop the fake-4K upscale on degraded footage.

**"Specialist hands" executors** (completing the Jasper-delegates-everything model): the whole Sales team (deal-closer, qualifier, objection-handler, outreach, merchandiser) and the Outreach department (email/SMS/voice) became operator-gated EXECUTORS — the LLM (GM verbatim) authors the human-facing note, deterministic code performs the CRM mutation / send, gated fail-closed on `viaApprovedMissionStep` (literal `true` only in the mission step-runner). Email send paths consolidated 4→2 canonical + 2 CAN-SPAM gaps closed. Live email delivered end-to-end; the live SMS send and a daytime voice call are the open verification items (see `CONTINUATION_PROMPT.md`).

**CRM (Vertical #2) — five remaining gaps built Jun 30; certification pending.** A code-trace of the bespoke pages confirmed custom fields (define in `/schemas` + render/edit on all 4 objects), record associations (contact↔company↔deal link + navigate), and the bespoke Leads list were DONE. **The five then-remaining gaps are now BUILT** (Jun 30, `a7a3275b` + follow-up), each via the real Admin-SDK data path: (1) **saved/filtered views + segmentation** — `savedViews` collection + `/api/crm/views` CRUD + real server-side filtering (`apply-view-filters.ts`, broad-fetch ≤1000 then filter) on all 4 list GET routes + `SavedViewsBar`/`FilterBuilderDialog`; (2) **multiple pipelines** — `pipelines` collection + `/api/crm/pipelines` CRUD, lazy default-pipeline seed (original 6 stages), `pipelineId` on deals with default fallback, board pipeline selector + dynamic stage columns, and a `/pipelines` management screen (create/rename/edit/delete; default un-deletable; stage keys never renamed); (3) **CSV import UI** — `ImportCsvModal` on contacts/companies/deals + three import routes mirroring the leads importer; (4) **editable record owner** — `OwnerSelect` on contacts/companies/leads/deals edit pages, `ownerId` whitelisted in PATCH; (5) **reporting in CRM** — `/crm-reports` reusing the real `/api/analytics/pipeline|forecast|win-loss` engines with honest empty states. `Pipelines` + `Reports` tabs added to `DEALS_TABS`. **Not yet certified** — each needs an operator walkthrough against Pipedrive/HubSpot before it counts as done. A duplicate-MERGE UI was built then REVERTED by operator decision — merge destroys records and fights the wanted model of one contact linked to MULTIPLE accounts, which is the foundation for the planned agency (reseller) + white-label tier (parked for research; see memory `project_agency_reseller_account_model`). Also fixed + DEPLOYED the `chatSessions` composite-index direction (status ASC + completedAt DESC) so the completed-conversations query works.

**Prior Last Updated:** June 22, 2026 (**Video: no-stitch realignment + generation hardening + character library-move fix; first true end-to-end video run proven on live fal**; commits `ceb6bdf1`→`16dcc0a2` on dev). The engine no longer stitches — each shot renders as its OWN clip and all clips flow into the editor in play order, where the operator assembles them (`docHasVideo` = every shot has a clip, not a stitched `finalVideoUrl`; `seedEditorFromProject` hands one clip per shot in order; the System B per-doc generate route dropped `stitchShotPlan`, which is kept only for the legacy path). `generateAllShots` is now resilient: a failed shot is retried once then FLAGGED (`generated.status='failed'` + `error`) and skipped — one bad shot no longer aborts the whole build; continue shots chain from the most recent good frame (`priorChainFrame`; degrade to a cut if none); progress persists after every shot. Creating a character from library images now MOVES them out of the regular library (`removeLibraryRecordsByUrls` in `createAvatarProfile`, scoped to skip category 'character' so AI sheets are safe) — no more library pollution. Lighting-swatch label clamped to its 200-char cap (was crashing that still). Proven by `scripts/verify-video-e2e-no-stitch.ts` (brief → System B project → 6 individual clips → editor handoff; fully green: 6/6, no stitch, clips in order). **KNOWN-CORRECTED:** the video front-door VP-C (`VideoScriptForm`) was falsely marked done and is NOT built — see `CONTINUATION_PROMPT.md`. tsc + lint clean.

**Prior Last Updated:** June 19, 2026 (**Shot Doc → OpenArt Smart Shot parity pass**; commits `fa724d58`→`2082bdea` on dev). The Video Shot Doc (`src/app/(dashboard)/content/video/components/ShotPlanDocument.tsx`) is a read-only "production sheet" the renderer PAINTS from the AI-authored `plan.layout` (rows/blocks/weights emitted by the `SHOT_PLAN_PLANNER` GM) — never a hardcoded template. **Governing split:** consistent craft = renderer "house style" (guaranteed on every sheet); story-dependent composition = the AI. **Renderer house-style guardrails:** every image cell fills (no white gaps); each character = 1 hero image + small thumbnail views; costume/props = a small captioned strip (never out-sizes the cast); the blocking-diagram row floored tall + width-dominant; redundant standalone notes/palette blocks dropped (already shown in the cast block); the assembled-prompt row floored to ~2–3 lines + scroll. **Blocking diagram** is a read-only, fully-visible view with in-place scroll-to-zoom + drag-pan (`BlockingZoom`, native non-passive wheel listener; not a popup); `FloorPlanCanvas` gained a `readOnly` mode (toolbar hidden, inert, SVG fits its container). **Editing model:** the doc is read-only and EVERY section edits via its OWN per-section popup (replaced the old whole-sheet Review/Edit toggle) — an `editingSection` state drives one controlled Dialog keyed by section: Shared choices / Cast & Props / Locations & Environment / Lighting-Mood-Style / Storyboard / Edit camera blocking; clicking a storyboard frame still opens the per-shot detail popup. **Planner GM:** the 7 OpenArt example sheets were studied and their layout craft DISTILLED INTO the GM `systemPrompt` as text expertise (story-adaptive sections, row-height budget, cast/costume/blocking grammar); runtime example-image few-shot is OFF (`LAYOUT_EXAMPLES = []` in `scripts/seed-shot-plan-planner-gm.js`) with the vision plumbing kept as a fallback (images parked in storage via `scripts/upload-layout-examples.js`). Reseed after GM edits: `node scripts/seed-shot-plan-planner-gm.js --force`. tsc + lint clean throughout.

**Earlier:** June 15, 2026 (**Hedra fully removed** — the video engine is now **fal / Seedance** via the **Shot Plan** flow). The entire legacy Hedra storyboard pipeline was deleted: `hedra-service.ts`, `hedra-capability-service.ts`, `hedra-prompt-agent.ts`, `hedra-prompt-translator.ts`, `scene-generator.ts`, the `HEDRA_SPECIALIST` agent, the old wizard steps (`StepGeneration`/`StepAssembly`/`StepPostProduction`/`StepPublish` + `SceneProgressCard`/`EngineSelector`/`HedraCharacterBrowser`), and the routes `/api/video/{generate-scenes, poll-scenes, regenerate-scene, scene-review, scene-preview/save, decompose, calendar/generate-all, assemble, stream, avatar-profiles/sync-hedra, avatar-profiles/hedra-characters}` (`/api/content/video/generate` is now inert and points operators to the Shot Plan tool). The live video flow is the **Shot Plan** tool (Content → Video) → renders shot-by-shot on fal / Seedance with last-frame chaining → hands clips to the standalone editor. Image generation + the Studio image/edit tools were repointed to fal (Flux + Flux Kontext); image auto-select defaults to fal. The `hedra` API-key field is replaced by **fal.ai (Seedance)**. Jasper's `produce_video`/`generate_video`/`assemble_video`/`get_video_status` tools now redirect to the Shot Plan tool. tsc + lint clean. Sections below that still describe "Hedra" as the live engine are historical and superseded by this entry.

**Prior Last Updated:** June 4, 2026 (verified against code, not prior docs). **CORRECTED AGENT COUNT from `src/lib/agents/agent-registry.ts`: 74 agents = 1 L1 + 10 L2 + 59 L3 + 4 STANDALONE** (prior docs said "72 / 57 specialists" — stale). `AGENT_REGISTRY.json` is broken/empty (0 ids) — regenerate from `agent-registry.ts`. **Server-SDK "wrong door" remediation (Step 1 of the May 21 plan — code-complete + pushed to dev, but NOT operator-tested, so per the owner's bar it is UNPROVEN, not done).** Systemic bug found: server code (API routes, crons, webhooks, and the lib services they call) used the CLIENT Firebase SDK, which has no auth on the server → Firestore rules silently reject → reads return []/null, writes denied, errors swallowed. Not "never worked": a half-finished Apr 30 migration (only Zoom + public early-access leads got the `useAdminSdk` fix) + scaffolding built-but-never-tested (Quotes/Invoices/Payments). Dashboard screens often write Firestore directly from the browser (authed → works), masking the broken duplicate server paths. Converted client→AdminFirestoreService across every broken-AND-used surface (commits `df5f5bf2`→`5daba17d`): TCPA STOP opt-out + meeting reminders; OAuth + Gmail (+ fixed malformed `organizations/*/oauthStates` read path); quote/invoice/payment + deal + contact services; forms builder (get/update/publishForm); leads createLead useAdminSdk (manual + public-form + import); lead routing (getLead/updateLead + lead-routing engine); enrichment + cache; voice TTS config; commerce specialists + manager; integration-manager view/update/sync/test/list; promotion-service + legacy training apply/deploy; notification-service (+ admin Timestamp). Each tsc+lint verified, several with full `npm run build`. Dead/"doesn't matter" list + fix pattern in CONTINUATION_PROMPT.md (note: gemini-service's client-SDK key read is dead on the server — keys resolve via Admin-first `api-key-service`, which is WHY AI works). **Tier 3 automation engines (`e160ab7a`, `9811f75d`):** master kill-switch `areAutomationsEnabled()` (`src/lib/automation/automation-gate.ts`, env `AUTOMATIONS_ENABLED`, default OFF) gates the 3 automation crons; converted the workflow + email-sequence EXECUTION path to Admin SDK so they run WHEN ENABLED (still OFF + untested). Remaining: workflow-service + workflow-executions-service need a client/server split (browser pages import them). **NEW FEATURE — Agent Model Management (`ae21d12c`→`f881ba75`):** operator changes which LLM each agent uses, tests it, and ROLLS BACK — a model change creates a new versioned Golden Master (same prompt + Brand DNA, only `config.model` differs) and deploys it; rollback reuses the version-deploy path. New `src/lib/training/agent-model-service.ts`, API `/api/admin/agent-models` (list/change/rollback), UI at **`/workforce/models`** ("Agent Models" tab in `AI_WORKFORCE_TABS`). All agents now read `config.model` (converted the 2 that hard-coded it: growth-strategist, marketing-manager). Operator model changes are exempt from the grade-gated prompt pipeline (Standing Rule #2 guards prompts, not model choice) but are versioned. Also fixed a double-emoji bug in the AI Agent Configuration sidebar. **PER-AGENT ISOLATED KNOWLEDGE BASES — PILOT FOUNDATION DONE + PROVEN (commit `583ff92b`):** each agent now has its OWN knowledge base, retrieved at runtime with no cross-agent bleed and no bleed into the global chat/Jasper RAG. `vector-search.ts` `searchKnowledgeBase`/`storeEmbedding`/`indexKnowledgeBase` take an optional `agentId` (canonical registry id, e.g. `COPYWRITER`) — per-agent doc id, `agentId`-tagged embeddings, stale-cleanup on re-index, and the global path filters OUT agent-tagged vectors; with no `agentId` the legacy global behavior is unchanged (backward compatible). New `rag-service.getAgentKnowledgeContext(agentId, query)` returns a prompt-injectable block. Pilot agent **Copywriter (`COPYWRITER`)** is wired to pull from its own KB on every action (page copy, proposal, email sequence) via `callOpenRouter`. New API `GET/POST/DELETE /api/agent/knowledge/[agentId]` (files, URLs, FAQs, pasted text; asset-type tagging; merges + re-indexes per agent; agent id validated against the registry). New UI at **`/workforce/agents/[agentId]/knowledge`** (upload / list / two-step-confirm delete), reached via a new "Knowledge" button on each Workforce agent card. PROVEN on real Firestore via `scripts/verify-per-agent-knowledge.ts`: agent A retrieves its own doc, agent B and the global path do NOT, recursive cleanup leaves zero residue — all 6 checks pass. **Embeddings now run on OpenAI (`text-embedding-3-small`, pinned to 768 dims), read from the central Firestore key via `apiKeyService` (commit `40f6c5ce`)** — `embeddings-service.generateEmbedding` tries OpenAI first, then Google, then the deterministic hash. The prior Gemini key returns errors for `embedContent`, so it was effectively on the hash fallback; OpenAI is the working central key (no client setup — same model as OpenRouter powering the agent brains). Proven semantic: related sentences 0.54 cosine, unrelated 0.04. **Video/audio upload works:** the per-agent route splits audio/video files off and transcribes them to text via Deepgram (`transcribeAudioBuffer` added — Deepgram accepts video containers directly, no ffmpeg), storing the transcript as a document; proven OpenAI-TTS→Deepgram round-trip returned the exact sentence. **Live output PROVEN:** `scripts/verify-copywriter-knowledge-live.ts` seeds an INVENTED product guarantee into the Copywriter's KB, runs the real Copywriter (live OpenRouter call), and the invented fact appears verbatim in the generated email copy — it exists nowhere else, so the full upload→embed→retrieve→inject→generate loop is proven (recursive cleanup, refuses to run if a real COPYWRITER KB exists). tsc + lint + full build clean. **KNOWN DATA ISSUE (separate):** the ElevenLabs key in Firestore is invalid (401) — voice/TTS features on that provider are on a dead key. **REMAINING:** rollout across the remaining 73 agents (same `callOpenRouter`-style wiring) — NOT until the operator confirms the pilot in a manual walkthrough.

**Prior Last Updated:** May 21, 2026 (penthouse-completion plan restructured to three parallel workstreams; broken email builder save path fixed; four competitive audits filed; three external platform clocks started). **Plan restructure** (CONTINUATION_PROMPT.md): owner quality-bar reset — "code-shipped is NOT done, competitive parity with a named incumbent is done." May 19 Sprint #1-#7 plan replaced with three parallel workstreams: A (connect every platform end-to-end), B (lift each surface from starter to competitive — Klaviyo for email, Buffer/Later for social, HubSpot for CRM, Stripe for onboarding, Webflow for landing pages, etc.), C (cutting-edge differentiators leveraging Brand DNA the incumbents don't have). Three workstreams run in parallel; multi-tenant flip only after all three are GREEN. **Orchestration mode standing rule** added: Claude orchestrates, subagents do brunt of work, Claude reviews every subagent output BEFORE declaring done. **New API route** (`src/app/api/email/templates/route.ts`): POST + GET for the email-builder save flow. Admin SDK + `requireAuth` + Zod schemas for `EmailTemplate` / `EmailBlock` / `EmailVariable`. Block-array templates persist to new `emailBuilderTemplates` Firestore collection (kept distinct from the existing HTML-body `emailTemplates` collection so block-array data round-trips into the editor cleanly). POST upserts on `id` and stamps `createdAt` only on first write; GET lists templates ordered by `updatedAt desc`. Verified live: previously-404 endpoint now returns 401 unauthenticated (route exists + auth gate firing). **Email builder page handler updated** (`src/app/(dashboard)/marketing/email-builder/page.tsx:292-322`): save handler now (a) reads the saved id back from the response so subsequent saves update the same record instead of duplicating, (b) surfaces backend error messages instead of silently swallowing `!response.ok`, (c) removed the broken redirect to `/marketing/templates` (that page doesn't exist). Unused `useRouter` import dropped. **Four competitive audits filed via parallel SaaS Architect + Architect subagents** (research-only, no code writes): (i) Klaviyo email template library + builder — 19 specific gaps with RICE scores; differentiators identified: Brand-DNA template generation + compliance send-gate. (ii) Buffer + Later social composer — 0/24 parity items, ~18-24 months behind on every dimension; differentiators: Brand-DNA AI captions + AI alt-text/thumbnail + cross-channel A/B. (iii) HubSpot Sales Hub Breeze AI for CRM — 9 missing / 2 basic / 1 superior (Brand DNA bake); differentiators: transparent deal scoring + multi-source enrichment + Shapley attribution. (iv) Email Builder code map — found broken save path (now fixed above), unreconciled dual `EmailTemplate` types, `@dnd-kit` already installed (Phase 4 DnD is cheap), Firebase Storage pattern exists in `media-service.ts` (Phase 5 upload is cheap), 671-line monolithic page needs sub-component extraction before Phase 4. **External clocks started** (Workstream A): Pinterest dev app submitted (App ID 1573185, Trial access pending, 1-3 day clock); LinkedIn dev app created (Client ID `86j1rfahybkhy8`, Client Secret saved to .env.local, redirect URL `https://salesvelocity.ai/api/social/oauth/callback/linkedin` registered, MDP review clock running); Google OAuth Verification form 95% complete (External + Production, 14 scopes registered with sensitive + restricted justifications + features dropdown — only pending the demo screencast). The old suspended SalesVelocity Google Business Profile is irrelevant — we use one OAuth bundle (`GOOGLE_FULL_SCOPE_BUNDLE` at `src/lib/integrations/google-tokens.ts:490`) that covers Gmail + Calendar + Drive + YouTube + Business Profile + Analytics + Search Console + Google Ads in a single tenant consent. tsc + eslint clean on changed files.

**Prior Last Updated:** May 7, 2026 (Sentry production monitoring wired; memory-vault crash fix; StepGradeWidget routing + error surfacing rebuilt; GM seeding audit script). **Sentry wiring finished:** `sentry.client.config.ts` created (server + edge configs were already present), DSN env vars (`SENTRY_DSN` + `NEXT_PUBLIC_SENTRY_DSN`) added to Vercel for Production+Preview+Development AND to local `.env.local`, `next.config.js` already wraps with `withSentryConfig` and externalizes `@sentry/nextjs` to keep function bundles under Vercel's 250 MB hard limit. Sentry SDK initialization confirmed on dev-server startup. **memory-vault crash fix** (`src/lib/agents/shared/memory-vault.ts:777-781`): `getPendingSignals` was calling `s.value.affectedAgents.includes(agentId)` without guarding against undefined; legacy SignalEntry records lacking the field were crashing every `MARKETING_MANAGER.checkIntelligenceSignals()` call (and surfacing on every Marketing Manager invocation per the prior open-issues list). Now defaults to `[]` when the field is missing or non-array. **StepGradeWidget rebuild** (`src/app/(dashboard)/mission-control/_components/StepGradeWidget.tsx`): two structural changes — (a) the picker now includes the **manager** as a target alongside specialists, derived from the step's `delegatedTo` field as `${delegatedTo}_MANAGER` (e.g. `MARKETING_MANAGER`), so cross-platform / orchestration corrections (multi-platform image consistency, channel-spanning tone rules) can land on the right prompt instead of being mis-routed to a single platform expert; (b) backend pipeline rejections (`/api/training/grade-specialist` returning non-OK, e.g. when `getActiveSpecialistGMByIndustry` finds no matching GM and `markFeedbackDiscarded` fires) are now surfaced via a dismissible `pipelineMessage` banner with `error|warn|info` styling — previously the widget silently swallowed `!gradeRes.ok` and the operator had no way to know their grade was discarded. CLARIFICATION_NEEDED status, missing-edit-text status, and try/catch failures all surface their own specific banner copy. The star rating still saves to `missionGrades` regardless — the banner only addresses the prompt-edit half. **`StepGradeWidget` props extended:** new optional `delegatedTo?: string` prop wired from `mission-control/page.tsx`. **GM seeding audit:** `scripts/audit-gm-seeding.ts` cross-references admin registry IDs (`SpecialistRegistry.tsx`), runtime delegation IDs (`marketing/manager.ts`), seed-script `SPECIALIST_ID` constants, and live `specialistGoldenMasters` Firestore docs against the active industry (`saas_sales_ops`). Findings: **10/10 manager GMs seeded** (`MARKETING_MANAGER`, `CONTENT_MANAGER`, `INTELLIGENCE_MANAGER`, `BUILDER_MANAGER`, `ARCHITECT_MANAGER`, `COMMERCE_MANAGER`, `OUTREACH_MANAGER`, `OPERATIONS_MANAGER`, `REPUTATION_MANAGER`, `REVENUE_DIRECTOR`); **55 specialist GMs live** for `saas_sales_ops`; **8 reported "missing"** are mostly admin/UI naming aliases over already-seeded canonical IDs — `X_EXPERT` aliases `TWITTER_X_EXPERT`, `WEB_SCRAPER` aliases `SCRAPER_SPECIALIST`, `ARCHITECT_COPY_STRATEGIST` aliases `COPY_STRATEGIST`, `ARCHITECT_FUNNEL_STRATEGIST` aliases `FUNNEL_STRATEGIST`, `ARCHITECT_UX_UI_STRATEGIST` aliases `UX_UI_STRATEGIST`, `REVENUE_DIRECTOR` is a manager (already seeded at manager level), `AUTONOMOUS_POSTING_AGENT` is deterministic automation (no LLM, no GM needed). **Only one truly missing specialist GM:** `PRICING_STRATEGIST` — but Standing-Rule audit shows the existing `PRICING_STRATEGIST` is a deterministic Stripe/payment dispatcher (`src/lib/agents/commerce/pricing/specialist.ts` — `systemPrompt: ''`, `tools: []`, no LLM call) so it doesn't need a GM either. The **budget-allocator agent** the operator has been describing (daily/weekly/monthly ad spend awareness, SEO/social/PPC/Local Service Ads allocation, ROI-based reallocation recommendations with operator-approved auto-apply via per-platform ad APIs) is a NEW agent under `MARKETING_MANAGER` — spec drafted in `CONTINUATION_PROMPT.md`, build TBD. **The actual root cause of the BYOK mission's silent grade discard** (`tfb_x_expert_1778182378512` discardedReason: "No active Golden Master found for X_EXPERT:saas_sales_ops") was the alias mismatch — runtime/admin layer emits `X_EXPERT`, seed/factory layer uses `TWITTER_X_EXPERT`. The StepGradeWidget surfacing fix lands in this commit; the alias-resolver fix in `getActiveSpecialistGMByIndustry` is queued for tomorrow's session. **Open-issues table updated:** the prior `checkIntelligenceSignals` line item is closed. tsc + eslint clean across all 4 modified files.

**Prior Last Updated:** May 3, 2026 (single-step social posting actually publishes; multi-platform fan-out shipped; Bluesky auto-compress; Social Hub duplicate header removed). **Architectural change to Marketing Manager:** `executeSinglePlatformPost` (`src/lib/agents/marketing/manager.ts`) now calls `autonomous-posting-agent.executeAction({ type: 'POST' })` after drafting + image resolution — previously it returned `COMPLETED` after just generating draft text and an image, leaving "complete" missions where no platform actually received a post. Mission Control's per-step result now includes `published: true`, `publishedActionId`, and `publishedPlatformActionId`. Failure path returns `FAILED` with `publishError` and (when applicable) `complianceBlocked: true` + `complianceReason` so silent-success-with-nothing-posted is no longer possible. **New `executeMultiPlatformPost` method** (same file): when `delegate_to_marketing` is called with `platforms: [...]` array OR `platform: "all"`, the manager loops the single-platform path per platform and returns aggregated per-platform results. `"all"` expands to active connected accounts via `SocialAccountService.listAccounts()`. Status is `COMPLETED` if at least one platform succeeded; data includes `successCount`, `totalCount`, `partial` flag, and per-platform `results[]` with `published`/`publishedPlatformActionId`/`error`. **`delegate_to_marketing` tool schema extended** (`src/lib/orchestrator/jasper-tools.ts:1630`): added `platforms` (array of strings) parameter so Jasper can request multi-platform posts in a single tool call. `marketingPayload` passes the array through. **Bluesky image auto-compression** (`src/lib/integrations/bluesky-service.ts:uploadBlobFromUrl`): when a JPEG/PNG/WebP image exceeds 1.9 MB (safe margin under Bluesky's 2 MB hard limit), the service auto-resizes via Sharp (`rotate()` honoring EXIF, 1600px max width, JPEG quality stepdown 85→40 with mozjpeg) before uploading. Operator no longer needs to know per-platform image limits. **Social Hub duplicate tab fix** (`src/app/(dashboard)/social/page.tsx`): removed the duplicate `<SubpageNav items={SOCIAL_TABS} />` from the page (the layout already renders it once for every social subroute). **Verified live** end-to-end on local: a single "Create a social media post for Summer 2026 kickoff" prompt produced a multi-platform post that landed on X (real tweet) and Mastodon (real toot) at ~92s total mission duration — real work, no more 2.2-second silent completes. **Shortcuts taken (named per standing rule):** Bluesky image compression patch was committed without retest after the resize logic was added — only the architectural fix (X + Mastodon) was end-to-end verified; `platform: "all"` expansion path was code-added but not specifically exercised in the test prompt (Jasper picked an explicit `platforms: [twitter, bluesky, mastodon]` array). **Known bugs surfaced but NOT fixed in this session** (logged for next session): (a) `scan_leads` Apollo industry/keyword/location filters silently dropped — every query returns the same 25 companies regardless of params (`{industry:"Lawncare"}` and `{industry:"Environmental Services"}` returned identical results, both with `totalResults=17,546,296`); (b) `/api/social/platforms/[platform]/audience` GETs require composite Firestore index on `audience_snapshots` (`accountId, platform, dayKey, __name__`) — Firestore Console one-click create-index URL is in the error log; (c) `findPendingMissionForPlatform` requires composite Firestore index on `missions` (`metadata.platform, status, createdAt`) — same one-click fix; (d) navigating to `/social/linkedin` (non-existent route) throws a React `useContext` 500 instead of a 404. **Standing rules preserved.** tsc + eslint clean across all 4 modified files.
...
- **fal / Seedance is the sole video engine.** Two generation modes:
  - **Text-to-video:** Seedance generates the character, environment, and native audio from the prompt alone. No reference needed.
  - **Reference-to-video:** a saved character is cast into the new scene from its reference images, with identity held across shots via last-frame chaining.
- The **Shot Plan** tool (Content → Video) is the sole creation surface: it writes a shot plan, renders each shot on fal / Seedance, and hands the clips to the editor.
- **Voices** for narration come from ElevenLabs, UnrealSpeech, and custom clones (`type: "voice_clone"`).
- **Character Studio** stores client characters in Firestore (portraits, voices, metadata) — the tenant's own digital cast.
...
- **Prompt pipeline:** `shot-plan-mapping.ts` composes each shot's generation prompt from the Shot Plan's shared choices + cinematic config for both text-to-video and reference-to-video modes.
...
- Multi-provider backend: Fal.ai (Flux — primary for images), Google Imagen 3, DALL-E 3, Kling 3.0 — fallback chain with retry-on-auth-failure
...
|----------|-----------|--------|------------|
| Fal.ai | Video (Seedance) + image gen | Seedance, Flux, SDXL, Flux Kontext | $0.01–$0.15 |
...
**What's ready:** Jasper orchestration (46 tools, 9 delegation routes, all real), Mission Control (SSE streaming, review links, approval gates), CRM (lead scoring, enrichment, deal pipeline), website builder (35+ widgets, AI generation, migration), workflows, forms, coaching, RBAC (100% route coverage — 366 standard auth + 50 alternative auth + 20 intentionally public), payments via Stripe (full lifecycle), social media (12 platforms wired into the post dispatcher with real API client code — Discord/Twitch are NOT in the dispatcher; only 3 are operator-connectable today: Twitter/X, Bluesky, Mastodon — the other 9 have real code but are gated `coming_soon` on external app approvals), onboarding (5-step wizard with auto-save), invoice generation (Playwright PDF), email unsubscribe route, billing settings page, team invite flow + role management UI, campaign CRUD (including DELETE), all 12 payment webhook handlers.
...
| Video System | **85% READY** — fal / Seedance real and working via the Shot Plan flow (reference-to-video + text-to-video, last-frame chaining), per-shot persistence to storage + media library, editor hand-off, FFmpeg assembly. Audio mixing in stitcher-service is real (segment selection, BPM detection, beat markers, fade config). **GAP:** Music library has 15 tracks with placeholder Firebase Storage URLs (need real audio files uploaded). |
...
| Video System | 8.5/10 | MOSTLY READY — fal / Seedance real (Shot Plan flow), per-shot generation, FFmpeg assembly, real audio mixing. **Gap: Music library has 15 tracks with placeholder Firebase Storage URLs.** |
...
| 15 | **Video system** | fal / Seedance engine via the Shot Plan flow (legacy Hedra pipeline removed Jun 15) | ✅ FIXED |
...
- `/pipelines` (NEW Jun 30 — multi-pipeline management: create/rename/edit stages/delete; Deals-nav tab)
- `/crm-reports` (NEW Jun 30 — in-CRM pipeline/forecast/win-loss reports; Deals-nav tab)
...
**Video & Voice Lab:**
- `/content/video` (Video Studio — the Shot Plan creation surface on fal / Seedance + Character Studio)
...
- **Character Studio** — reusable character library: the operator's own created characters, reference images, green screen clips, voice assignment, role/style tags.
- **Shot Plan** — the creation surface: per-shot fields, cast, reference materials → renders each shot on fal / Seedance (last-frame chaining) → hands clips to the editor. `edit_video` + `manage_media_library` Jasper tools remain; `produce_video`/`assemble_video` now redirect to the Shot Plan tool.
- **Current Status: FUNCTIONAL** — fal / Seedance sole engine via the Shot Plan flow. Video Editor + Media Library operational.
...
#### Video & Voice Lab — fal / Seedance (Shot Plan)

**Current Status: FUNCTIONAL** — fal / Seedance sole engine via the Shot Plan flow. Character Studio, Standalone Video Editor, and Media Library all operational. The legacy Hedra storyboard routes (`generate-scenes`, `poll-scenes`, `regenerate-scene`, `scene-review`, `decompose`, `calendar/generate-all`, `assemble`, `stream`, `avatar-profiles/sync-hedra`, `avatar-profiles/hedra-characters`) were REMOVED.

| Endpoint | Method | Purpose | Status |
|----------|--------|---------|--------|
| `/api/video/avatar-profiles` | GET/POST | Character profile CRUD (the operator's own created characters) | FUNCTIONAL |
| `/api/video/avatar-profiles/[profileId]` | GET/PATCH/DELETE | Single profile ops | FUNCTIONAL |
| `/api/video/voices` | GET | List ElevenLabs / UnrealSpeech / custom voices | FUNCTIONAL |
...
| `/api/content/shot-plan/generate-shot` | POST | Render ONE shot on fal / Seedance, persist clip + last frame | FUNCTIONAL |
| `/api/content/shot-plan/generate-all` | POST | Render every shot in the plan, chained | FUNCTIONAL |
| `/api/content/shot-plan/generate-keyframe` | POST | Cheap pre-video still for a shot | FUNCTIONAL |
...
**API Keys to Firestore:** All service API keys (OpenRouter, SendGrid, Stripe, Twilio, ElevenLabs, Serper, DataForSEO, Twitter/X, Slack, PageSpeed) are now stored in Firestore via `apiKeyService` rather than `.env` files. `process.env` remains as fallback for development and for system secrets (webhook verification keys, Firebase admin credentials, encryption keys).
...
| **Video (fal / Seedance)** | **REAL** | fal / Seedance — sole video engine via the Shot Plan flow (reference-to-video + text-to-video, last-frame chaining). Character Studio (custom), narration via ElevenLabs/UnrealSpeech, editor hand-off, brand preference memory. |
