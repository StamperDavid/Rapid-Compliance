...
**Last Updated:** April 24, 2026 (evening) — **Stage A.5 shipped: `create_workflow` tool live end-to-end.** workflow-001 + workflow-002 now pass 2/2 via `scripts/verify-prompt-matrix-e2e.ts --category=workflow`. New tool schema + executor in `src/lib/orchestrator/jasper-tools.ts`, new `src/lib/workflows/sequence-scheduler.ts` (per-email sendTimingHint → day offsets, scheduleEmailSequence batch writer, fireReadySequenceJobs cron dispatcher), new `WorkflowSequenceJob` type, `/api/cron/workflow-scheduler` fires due jobs every 5 min (registered in vercel.json). Workflow doc written via adminDb directly (client FirestoreService returns PERMISSION_DENIED server-side). Verified: 5-email nurture over 14 days produces a Workflow doc + 5 sequenceJobs with day 1/3/7/10/14 staggering. Firestore composite indexes declared for `workflowSequenceJobs` (status+fireAt, workflowId+stepIndex) — deploy via `firebase deploy --only firestore:indexes`. No GM edits (Standing Rule #2 respected). **Prior — April 24 midday:** E2E runner shipped (`scripts/verify-prompt-matrix-e2e.ts`) + Content Manager EMAIL_SEQUENCE intent rebuild + Copywriter v2 (generate_email_sequence action). **Prior — April 23, 2026:** Jasper v11 call-shape fix + parser hardening. Matrix planning regression 242/245 (99%) at 5 iter. **Prior — April 15, 2026:** Mission Control M1-M8 complete — plan pre-approval, sequential execute with retry, downstream-changed flag, manual edit, inline scrap, full training loop all live.
**Branches:** `dev` (latest)
