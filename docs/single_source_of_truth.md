...
**Last Updated:** June 29, 2026 (**Navigation overhaul — Settings dissolved into the main menu; duplicates, orphans, and dead redirect-stubs purged. + Video Tier-1 cinematic fixes (frozen-tail + sound). + the Sales/Outreach "specialist hands" executors.** Commits on dev `112c3484`→`56b20bd8`.)
...
**CRM (Vertical #2) — verified state + a reverted experiment (Jun 29).** A code-trace of the bespoke pages confirms custom fields (define in `/schemas` + render/edit on all 4 objects), record associations (contact↔company↔deal link + navigate), and the bespoke Leads list are DONE; the real remaining gaps are saved/filtered views, multiple pipelines, CSV import UI, editable record owner, and reporting surfaced inside the CRM (full list in `CONTINUATION_PROMPT.md`). A duplicate-MERGE UI was built then REVERTED by operator decision — merge destroys records and fights the wanted model of one contact linked to MULTIPLE accounts, which is the foundation for the planned agency (reseller) + white-label tier (parked for research; see memory `project_agency_reseller_account_model`). Also fixed + DEPLOYED the `chatSessions` composite-index direction (status ASC + completedAt DESC) so the completed-conversations query works.
