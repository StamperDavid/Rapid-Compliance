...
async function main() {
  ...
  console.log(`\n${line}`);
  console.log('EMAIL SPECIALIST REALITY TEST — THE PIRATE TEST');
  console.log(line);
  ...
  if (!snap.exists) {
    console.error(`✗ GM doc ${GM_ID} not found. Seed the Email Specialist GM first.`);
    process.exit(1);
  }
  const original = snap.data();
  const originalPrompt = original.config.systemPrompt;
  const originalSnapshot = original.systemPromptSnapshot;
  const originalNotes = original.notes ?? 'v1 Email Specialist';
  ...
  console.log('\nStep 3: Running the proof-of-life harness (saas_cold_intro case)...');
  console.log('         (real OpenRouter call, will take ~1-2 minutes)');
  console.log(line);

  let result;
  try {
    result = spawnSync('npx', ['tsx', 'scripts/test-email-specialist.ts', '--case=saas_cold_intro'], {
  ...
  console.log('in subjectLine, previewText, bodyPlainText, ctaLine, psLine,');
  console.log('toneAndAngleReasoning, personalizationNotes, followupSuggestion,');
  console.log('spamRiskNotes, and rationale — the specialist is REAL.');
  console.log('(emailPurpose is a slug from the Firestore taxonomy and stays');
