/**
 * LinkedIn Expert — Proof of Life Harness
 *
 * Runs the rebuilt LinkedIn Expert end-to-end against live Firestore +
 * live OpenRouter and prints every layer so you can verify delegation is real.
 *
 * Usage:
 *   npx tsx scripts/test-linkedin-expert.ts
 *   npx tsx scripts/test-linkedin-expert.ts --case=saas_thought_leadership
 *   npx tsx scripts/test-linkedin-expert.ts --case=realestate_lead_gen
 *   npx tsx scripts/test-linkedin-expert.ts --case=agency_carousel
 *   npx tsx scripts/test-linkedin-expert.ts --raw   # also dumps raw OpenRouter body
 *
 * Exit codes:
...
import { getLinkedInExpert, __internal } from '../src/lib/agents/marketing/linkedin/specialist';
...
type CaseName = 'saas_thought_leadership' | 'realestate_lead_gen' | 'agency_carousel';
...
function parseFlags(): CliFlags {
  ...
  let caseName: CaseName = 'saas_thought_leadership';
  let showRaw = false;
  for (const arg of argv) {
    if (arg.startsWith('--case=')) {
      const value = arg.slice('--case='.length);
      if (value === 'saas_thought_leadership' || value === 'realestate_lead_gen' || value === 'agency_carousel') {
        caseName = value;
      } else {
        console.error(`Unknown --case value: ${value}. Supported: saas_thought_leadership | realestate_lead_gen | agency_carousel`);
        process.exit(1);
      }
    } else if (arg === '--raw') {
      showRaw = true;
    } else if (arg === '--help' || arg === '-h') {
      console.log('Usage: npx tsx scripts/test-linkedin-expert.ts [--case=saas_thought_leadership|realestate_lead_gen|agency_carousel] [--raw]');
      process.exit(0);
    }
  }
...
async function main(): Promise<void> {
  ...
  header(`LINKEDIN EXPERT PROOF OF LIFE — ${new Date().toISOString()}`);
  ...
  if (!gmRecord) {
    fail('Load GM', new Error(
      ...
      'Run "node scripts/seed-linkedin-expert-gm.js" first.',
    ));
  }
  ...
  section('[5/7] CALLING LINKEDIN EXPERT (real OpenRouter invocation)');
  const specialist = getLinkedInExpert();
  ...
  const validation = __internal.LinkedInContentResultSchema.safeParse(report.data);
  if (!validation.success) {
    fail('Zod validation', new Error(
      validation.error.issues.map((i) => `${i.path.join('.')}: ${i.message}`).join('; '),
    ));
  }
  console.log('  ✓ Output matches LinkedInContentResultSchema');
  console.log(`  ✓ post.content length: ${validation.data.post.content.length} chars`);
  console.log(`  ✓ post.hook: "${validation.data.post.hook.slice(0, 80)}..."`);
  console.log(`  ✓ post.callToAction: "${validation.data.post.callToAction}"`);
  console.log(`  ✓ post.hashtags: ${validation.data.post.hashtags.join(', ')}`);
  console.log(`  ✓ post.estimatedEngagement: ${validation.data.post.estimatedEngagement}`);
  console.log(`  ✓ post.bestPostingTime: ${validation.data.post.bestPostingTime}`);
  console.log(`  ✓ contentStrategy length: ${validation.data.contentStrategy.length} chars`);
  console.log(`  ✓ alternativeAngles.length=${validation.data.alternativeAngles.length}`);
  for (const angle of validation.data.alternativeAngles) {
    console.log(`      - angle="${angle.angle.slice(0, 60)}..." rationale="${angle.rationale.slice(0, 60)}..."`);
  }
