...
function getWorkflowExecutions(): SeedDoc[] {
  const execDefs = [
    ...
    { wfId: 'demo-workflow-002', wfName: '(Demo) Daily Pipeline Report', status: 'running', trigger: 'schedule', daysBack: 0, error: null, results: [{ action: 'generate_report', status: 'running', output: 'Generating pipeline summary...' }] },
    { wfId: 'demo-workflow-002', wfName: '(Demo) Daily Pipeline Report', status: 'running', trigger: 'schedule', daysBack: 1, error: null, results: [{ action: 'generate_report', status: 'running', output: 'Aggregating deal data...' }] },
  ...
  return [
    {
      ...
      status: 'IN_PROGRESS',
      userPrompt: '(Demo) Analyze our top 5 competitors and generate a comparison report',
      conversationId: 'demo-mission-conv-001',
      steps: [
        { stepId: 'step-1', toolName: '(Demo) Identify top 5 competitors', delegatedTo: 'research-agent', status: 'COMPLETED', startedAt: daysAgo(3).toDate().toISOString(), completedAt: daysAgo(2).toDate().toISOString(), summary: 'HubSpot, Salesforce, Pipedrive, Close.com, Apollo.io' },
        { stepId: 'step-2', toolName: '(Demo) Analyze feature parity', delegatedTo: 'research-agent', status: 'RUNNING', startedAt: hoursAgo(6).toDate().toISOString() },
        { stepId: 'step-3', toolName: '(Demo) Generate comparison report', delegatedTo: 'report-agent', status: 'PENDING', startedAt: '' },
      ],
      createdAt: daysAgo(3),
    ...
    },
    {
      ...
      status: 'COMPLETED',
      userPrompt: '(Demo) Clean up stale Q1 pipeline deals',
      conversationId: 'demo-mission-conv-002',
      steps: [
        { stepId: 'step-1', toolName: '(Demo) Identify stale deals (>30 days no activity)', delegatedTo: 'pipeline-agent', status: 'COMPLETED', startedAt: daysAgo(10).toDate().toISOString(), completedAt: daysAgo(8).toDate().toISOString(), summary: '12 stale deals identified' },
        { stepId: 'step-2', toolName: '(Demo) Close dead deals and update reasons', delegatedTo: 'pipeline-agent', status: 'COMPLETED', startedAt: daysAgo(8).toDate().toISOString(), completedAt: daysAgo(7).toDate().toISOString(), summary: '7 deals closed as lost' },
        { stepId: 'step-3', toolName: '(Demo) Reassign active deals to current reps', delegatedTo: 'assignment-agent', status: 'COMPLETED', startedAt: daysAgo(7).toDate().toISOString(), completedAt: daysAgo(6).toDate().toISOString(), summary: '5 deals reassigned' },
        { stepId: 'step-4', toolName: '(Demo) Generate cleanup summary report', delegatedTo: 'report-agent', status: 'COMPLETED', startedAt: daysAgo(6).toDate().toISOString(), completedAt: daysAgo(5).toDate().toISOString(), summary: 'Report saved to shared drive' },
      ],
  ...
  return [
    {
      ...
      enabled: true,
      routingType: 'skill-based',
      assignedUsers: ['demo-member-002'],
      priority: 1,
      conditions: [
    ...
    },
    {
      ...
      assignedUsers: ['demo-owner-001', 'demo-member-002', 'demo-member-003'],
      priority: 2,
      conditions: [
  ...
  return [
    {
      ...
      message: 'Hi {{firstName}}, this is a reminder about your appointment with {{repName}} at {{time}} tomorrow. Reply CONFIRM to confirm or RESCHEDULE to pick a new time. - SalesVelocity.ai',
    ...
    },
    {
      ...
      message: 'Hi {{firstName}}, thanks for joining the demo today! Here is the recording: {{recordingLink}}. Any questions? Just reply here or book a call: {{bookingLink}} - {{repName}}',
...
function getEmailTemplates(): SeedDoc[] {
  ...
  return templates.map(t => ({
    ...
    description: t.desc,
    subject: t.subject,
    html: `<html><body><h1>${t.subject}</h1><p>(Demo) Email body content for ${t.name}.</p></body></html>`,
    blocks: [],
    preview: `(Demo) Preview of ${t.name}`,
...
function getStorefrontConfig(): SeedDoc[] {
  return [{
    ...
    enabled: true,
    businessType: 'both',
    storeName: '(Demo) SalesVelocity.ai Store',
    storeUrl: 'https://store.salesvelocity.ai',
    theme: {
      primaryColor: '#3B82F6',
...
function getNotifications(): SeedDoc[] {
  const notifDefs = [
    { idSuffix: '001', cat: 'deal_updates', prio: 'high', channels: ['in_app'], status: 'read', userId: DEMO_OWNER_ID, title: '(Demo) Deal Won: Enterprise Plan - Acme', body: 'Congratulations! The Acme Corp deal ($45,000) has been marked as Closed Won.', actionUrl: '/deals/demo-deal-001', daysBack: 1, isRead: true },
    { idSuffix: '002', cat: 'lead_updates', prio: 'medium', channels: ['email', 'in_app'], status: 'sent', userId: MEMBER_2.id, title: '(Demo) New Lead Assigned: Horizon Media', body: 'A new lead has been assigned to you by round-robin routing.', actionUrl: '/leads/demo-lead-009', daysBack: 2, isRead: false },
    { idSuffix: '003', cat: 'task_reminders', prio: 'high', channels: ['in_app'], status: 'pending', userId: DEMO_OWNER_ID, title: '(Demo) Task Overdue: Follow up with Acme Corp', body: 'Your task "Follow up with Acme Corp on enterprise pricing" is past due.', actionUrl: '/tasks/demo-task-001', daysBack: 0, isRead: false },
    { idSuffix: '004', cat: 'sequence_updates', prio: 'low', channels: ['slack', 'in_app'], status: 'sent', userId: MEMBER_2.id, title: '(Demo) Sequence Completed: Enterprise Outreach', body: 'The Enterprise Outreach Sequence has completed for Rachel Lee.', actionUrl: '/sequences/demo-sequence-001', daysBack: 3, isRead: true },
    { idSuffix: '005', cat: 'system', prio: 'low', channels: ['in_app'], status: 'pending', userId: DEMO_OWNER_ID, title: '(Demo) System Update: Platform v2.4 Released', body: 'New features include enhanced AI scoring, workflow templates, and performance improvements.', actionUrl: '/settings/changelog', daysBack: 0, isRead: false },
  ];

  return notifDefs.map(n => ({
    id: `demo-notif-${n.idSuffix}`,
    userId: n.userId,
    templateId: `tpl-${n.cat}`,
    category: n.cat,
    priority: n.prio,
    channels: n.channels,
    status: n.status,
    variables: { userId: n.userId },
    content: {
      inApp: { title: n.title, body: n.body, actionUrl: n.actionUrl },
    },
    delivery: {
      attempts: {},
      lastAttempt: {},
    ...
    retry: { maxAttempts: 3, backoffMultiplier: 2, nextRetryAt: null },
    metadata: {
      createdAt: daysAgo(n.daysBack),
      updatedAt: daysAgo(Math.max(0, n.daysBack - 1)),
      read: n.isRead,
  ...
  return [
    {
      ...
      max_uses: 500,
      current_uses: 47,
      valid_from: daysAgo(30).toDate().toISOString(),
      valid_until: daysFromNow(60).toDate().toISOString(),
      ...
      applies_to: 'all',
      status: 'active',
      notes: '(Demo) Public marketing coupon — 20% discount on all products.',
      created_by: DEMO_OWNER_ID,
      isDemo: true,
      created_at: daysAgo(30).toDate().toISOString(),
      updated_at: daysAgo(5).toDate().toISOString(),
    },
    {
      id: 'demo-coupon-002',
      code: 'VIPSAVE50',
      ...
      max_uses: 100,
      current_uses: 12,
      valid_from: daysAgo(20).toDate().toISOString(),
      valid_until: daysFromNow(90).toDate().toISOString(),
      coupon_category: 'negotiation',
      ai_authorized: true,
      ai_discount_limit: 15,
      ai_auto_apply: false,
      applies_to: 'all',
      status: 'active',
      notes: '(Demo) Negotiation coupon for VIP prospects — $50 off.',
      created_by: DEMO_OWNER_ID,
      isDemo: true,
      created_at: daysAgo(20).toDate().toISOString(),
      updated_at: daysAgo(3).toDate().toISOString(),
    },
  ];
}
...
function getAuditLogs(): SeedDoc[] {
  const logs: Array<{
    ...
    { idSuffix: '001', action: 'user.login', userName: DEMO_OWNER_NAME, details: '(Demo) Successful login via Google OAuth.', ipAddress: '192.168.1.10', daysBack: 0 },
    { idSuffix: '002', action: 'deal.created', userName: MEMBER_2.name, details: '(Demo) Created deal "Enterprise Plan - Acme" ($45,000).', ipAddress: '10.0.0.25', daysBack: 1 },
    { idSuffix: '003', action: 'lead.updated', userName: MEMBER_3.name, details: '(Demo) Updated lead status from "new" to "qualified".', ipAddress: '172.16.0.33', daysBack: 2 },
    { idSuffix: '004', action: 'settings.changed', userName: DEMO_OWNER_NAME, details: '(Demo) Updated notification preferences: enabled Slack integration.', ipAddress: '192.168.1.10', daysBack: 5 },
    { idSuffix: '005', action: 'api_key.rotated', userName: DEMO_OWNER_NAME, details: '(Demo) Rotated API key for OpenAI integration. Old key invalidated.', ipAddress: '192.168.1.10', daysBack: 7 },
  ];
