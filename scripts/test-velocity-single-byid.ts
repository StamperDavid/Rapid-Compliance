/**
 * CLEAN single-character test using a specific library asset (a single clean
 * Velocity portrait). Places him in a grocery store via happy-horse, preserving
 * his comic art style, and saves the result to the library for review.
 * Run: npx tsx scripts/test-velocity-single-byid.ts
 */
...
import { getAsset, createAsset } from '../src/lib/media/media-library-service';
...
const ASSET_ID = '241be87d-7fcc-4920-8e89-ee0062b86916';
const sleep = (ms: number): Promise<void> => new Promise((r) => setTimeout(r, ms));

async function main(): Promise<void> {
  const src = await getAsset(ASSET_ID);
  if (!src?.url) { console.log(`Asset ${ASSET_ID} not found / no url.`); process.exit(1); }
  console.log(`Start frame: ${src.name}\n  ${src.url}`);
  ...
  const res = await generateWithHedra({
    ...
    type: 'video',
    textPrompt:
      'The exact comic-book superhero character shown in the reference image — keep his identical face, costume, the logo on his chest, colors, and his stylized comic-book / illustrated art style — walking down the aisle of a realistic grocery store, looking around. Do NOT convert him into a realistic human; preserve his original illustrated comic art style exactly.',
    startFrameUrl: src.url,
    resolution: '720p',
    aspectRatio: '16:9',
    durationMs: 5000,
  });
  ...
  const asset = await createAsset({
    ...
    name: 'TEST 3 - Velocity single clean portrait in grocery store',
    description: 'happy-horse single-character: clean Velocity portrait placed in a grocery store. Review: identity kept? comic style preserved or realified?',
